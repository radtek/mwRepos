...
#define MAX_PHONE_NUM				1000		//最多支持群发的手机个数
...
#define MAX_LOG_LEVEL_DEFAULT		0			//默认日志级别0   记录简单的出错信息或正常的登陆等信息
#define MAX_LOG_LEVEL_LOW			1			//低日志级别1     记录收发网络包
...
#define MAX_LOG_LEVEL_HIGHEST		5			//最高日志级别5	  记录非常详细的包
...
//	BOOL	m_bStopMt0Send;
	...
	int     m_nSacnfWaitTime; //读取滞留信息时间间隔
	int     m_nDealMtThrCnt;
	int     m_nDealUserCnt;
...
struct PACK_SUBMIT
{
	...
	int				nEcId;						//ecid
	int				nTaskId;					//任务Id
	short			nSplitFlag;					//该包是否已拆（0:不拆,1:全拆）
	int				nFeeFlag;					//费率标志
	char			cDealStatus;				//0默认状态 1:需做审核处理的新到状态 2:人工审核未通过 3:人工审核通过
	char			cStrLen;					//签名的字节长度
	unsigned char	ucLongMsgSeq;				//长短信协议头中的唯一标识
	short			nTotalMsgCount;
	short			nSendStatus;				/*	
	...
	short			nFlag;						//内部状态标志 
												...
												5:投递给后端SpGate失败
												*/
	USHORT			usInSendTimeSpan;			//是否在发送时间段内
	USHORT			usReSendNum;				//已重发次数
	UCHAR			ucExChannel;				//通道是否为主端口扩展  0:该通道直接从移动定购的由移动签名 1:在移动定购通道的基础上我司再增加的扩展
	USHORT			usSinglelen;
	USHORT			usMultilen1;
	USHORT			usMultilen2;
	USHORT			usSignlen;
	short			nLongMsg;					//0:非长短信 1:长短信
	...
	USHORT			nVer;						//1字节//双方协商的版本号(高位4bit表示主版本号,低位4bit表示次版本号)
	UINT			nTimeStamp;					//4字节//时间戳的明文,由客户端产生,格式为MMDDHHMMSS，即月日时分秒，10位数字的整型，右对齐 。
	USHORT			nResult;					//1字节//结果
	UCHAR			szSpGate[MAX_CHANNEL_LEN+1];				//spgate
	...
	USHORT			nPkTotal;					//1字节//相同Msg_Id的信息总条数，从1开始 
	USHORT			nPkNumber;					//1字节//相同Msg_Id的信息序号，从1开始
	USHORT			nReportFlag;				//1字节//是否要求返回状态确认报告：0：不需要1：需要2：产生SMC话单 （该类型短信仅供网关计费使用，不发送给目的终端)
	short			nMsgLevel;					//1字节//信息级别
	UCHAR			szServiceId[MAX_SRVID_LEN+1];			//10字节//业务类型，是数字、字母和符号的组合。
	USHORT			nFeeUserType;				//1字节//计费用户类型字段0：对目的终端MSISDN计费；1：对源终端MSISDN计费；2：对SP计费;3：表示本字段无效，对谁计费参见Fee_terminal_Id字段。
	UCHAR			szFeeTimernateId[MAX_FEETMID30_LEN+1];		//21字节//被计费用户的号码（如本字节填空，则表示本字段无效，对谁计费参见Fee_UserType字段，本字段与Fee_UserType字段互斥）
	USHORT			nFeeTerminalType;
	USHORT			nTpId;						//1字节//GSM协议类型。详细是解释请参考GSM03.40中的9.2.3.9
	USHORT			nTpUdhi;					//1字节//GSM协议类型。详细是解释请参考GSM03.40中的9.2.3.23,仅使用1位，右对齐
	USHORT			nMsgFmt;					//1字节//信息格式  0：ASCII串  3：短信写卡操作  4：二进制信息  8：UCS2编码15：含GB汉字  。。。。。。
	UCHAR			szMsgsrc[MAX_SPID_LEN+1];				//6字节//信息内容来源(SP_Id)
	UCHAR			szFeeType[MAX_FEETYPE_LEN+1];				//2字节//资费类别01：对"计费用户号码"免费02：对"计费用户号码"按条计信息费03：对"计费用户号码"按包月收取信息费04：对"计费用户号码"的信息费封顶05：对"计费用户号码"的收费是由SP实现
	UCHAR			szFeeCode[MAX_FEECODE_LEN+1];				//6字节//资费代码（以分为单位）
	UCHAR			szValidTime[MAX_VALIDTIME_LEN+1];			//17字节//存活有效期，格式遵循SMPP3.3协议
	UCHAR			szAtTime[MAX_ATTIME_LEN+1];				//17字节//定时发送时间，格式遵循SMPP3.3协议
	...
	UCHAR			szDestTerminalId[/*MAX_DESTTMID30_LEN*MAX_PHONE_NUM+*/1];	//21*nDestUsrTl字节//接收短信的MSISDN号码
	USHORT			nDestTerminalType;
