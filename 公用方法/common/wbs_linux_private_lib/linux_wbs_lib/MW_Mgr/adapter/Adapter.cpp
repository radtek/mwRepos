    ...
    if(!fWaitAll)
    {
		...
		if (bRet)
		{
			...
			for(iLoop = 0; iLoop < nCount; ++iLoop)
			{
				...
				if (bRet)
				{
					mEvent.SetMEvent(NULL, iLoop);
					mEvent.SetCompatibleEvent();
				}
			}
...
int  CAdapter::AsciiToUS2(const char* Msg, unsigned char *Data, unsigned int *Len)
{
    ...
    try
    {
        MultiByteToWideChar((UINT)CP_ACP,(DWORD)MB_PRECOMPOSED,(const char*)Msg, strlen(Msg), (LPWSTR)temp, sizeof(temp));
        int len = wcslen(temp);
        for (int i=0;i<len;++i)
        {
            int iFirst = 1;
