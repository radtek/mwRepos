...
public:
    ...
    static std::string format(const char *fmt, ...)
    {
		...
		static void format(std::string& strRet, const char *fmt, ...)
		{
			...
			size_t nSize = vsnprintf((char*)strRet.data(), nLength + 1, fmt, marker);
			va_end(marker);
			strResult.resize(nSize);
		}
...
static bool IsSpecialAscii(char cAscii);
	...
	//2014-10-13  qiwei  新增解析关键字函数
	static int GetCmdValue(char * phead,int nlen, int MaxSize, char *CmdVal, BOOL bUTF8=true);
	static int GetCmdValueEx( char * phead,int nlen, int MaxSize, char *CmdVal,BOOL bConvert=true);
	static int GetCmdValueExEx(char * phead,int nlen, int MaxSize, char *CmdVal);
	...
	static int SplitStrEx(const char * szSrc, std::vector<std::string> &vDest, const char* c, int nMaxLen);
