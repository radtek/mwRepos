...
/// process 64 bytes
void SHA256_EX::processBlock(const void* data)
{
  // get last hash
  uint32_t a = m_hash[0];
  uint32_t b = m_hash[1];
...
void SHA256_EX::add(const void* data, size_t numBytes)
{
...
void SHA256_EX::processBuffer()
{
  ...
  uint64_t_sh msgBits = 8 * (m_numBytes + m_bufferSize);
  // find right position
...
void SHA256_EX::getHash(unsigned char buffer[SHA256_EX::HashBytes])
{
...
std::string SHA256_EX::operator()(const void* data, size_t numBytes)
{
  reset();
  add(data, numBytes);
  return getHash();
...
std::string SHA256_EX::operator()(const std::string& text)
{
  reset();
