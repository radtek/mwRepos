...
int CAdapter::WaitForCompatibleEvent(CCompatibleEvent &event, DWORD dwMilliseconds)
{
	...
	timespec waittime = { 0 , 0 };
	clock_gettime(CLOCK_MONOTONIC, &waittime);
	waittime.tv_sec += nSec;
	waittime.tv_nsec += nMillisec*1000*1000;
    ...
    if(!event.IsSignaled())
    {
		pthread_mutex_lock(&event.GetMutex());
        int iWaitRet = pthread_cond_timedwait(&event.GetEvent(), &event.GetMutex(), (const struct  timespec*)&waittime);
		pthread_mutex_unlock(&event.GetMutex());
        if(0 == iWaitRet)
        {
            iRet = WAIT_OBJECT_0;
        }
             ...
             )
{
    ...
    if(!fWaitAll)
    {
        ...
        bRet = InitCompatibleEvent(mEvent, false);
		...
		if (bRet)
		{
			...
			{
				bRet = lppHandles[iLoop]->SetMEvent(&mEvent, iLoop);
				//如果事件组中存在有信号的事件,将代替事件置为有信号状态
				if (bRet)
				{
					bHaveEvent = true;
					dwRet = WAIT_OBJECT_0 + iLoop;
					break;
				}
			}
			...
			if (!bHaveEvent)
			{
				...
				if(WAIT_OBJECT_0 == iWaitRet)
				{
					...
					nReset = mEvent.GetMEvent()->me_nNo;
				}
				...
				//等待事件后，对事件是否需要手动重置进行处理
				if (bHaveEvent 
					&& dwRet >= WAIT_OBJECT_0 && dwRet < nCount
					&& 0 <= nReset && nReset < nCount)
				{
					//若是手动清信号则强制重新置为有信号
					if (lppHandles[nReset]->IsManualReset())
					{
						lppHandles[nReset]->SetCompatibleEvent();
					}
					else
					{
						lppHandles[nReset]->ResetSignalCnt();
					}
				}
			}
...
void CAdapter::SetEvent(CCompatibleEvent &event)
{
    event.SetCompatibleEvent();
    return;
}
...
bool CAdapter::CreateCompatibleThread(THREAD_RETURN(__STDCALL *start_address )( void * ), void *arglist, unsigned int stack_size)
{
//  char szBuf[100] = {0};
//  sprintf(szBuf, "Create Thread %d", iCount++);
//  CLogMgr::GetInstance().WriteLog(szBuf, "test.txt");
    int iRet = CCompatibleThread::CreateThread(start_address, arglist, stack_size);
    return iRet == 0;
}


bool CAdapter::CreateCompatibleThread(THREAD_RETURN(__STDCALL *start_address )( void * ), void *arglist, pthread_t& ptid)
{
    int iRet = CCompatibleThread::CreateThread(start_address, arglist, ptid);
    return iRet == 0;    
}
...
int CAdapter::Memicmp(const void* s1, const void* s2, size_t size)
{
	int f = 0;
	int l = 0;
	while (size--)
	{
		if ((*(unsigned char *)s1 == *(unsigned char *)s2) ||
			((f = toupper(*(unsigned char *)s1)) ==
			(l = toupper(*(unsigned char *)s2))))
		{
			s1 = (char *)s1 + 1;
			s2 = (char *)s2 + 1;
		}
...
void CAdapter::GetCurrentPath(char* szPath, int nMaxSize)
{
    ...
    */
    GetModuleFileName(szPath, nMaxSize);
	std::string strPath(szPath);
	size_t found = strPath.find_last_of("/");
	memset(szPath + found + 1, 0, strPath.size() - found - 1);
#endif
...
void CAdapter::GetModuleFileName(char* szPath, int nMaxSize)
{
	//WINDOWS编译环境
#if (defined  MW_WINDOWS) || (defined  WIN32)
	::GetModuleFileName(NULL, szPath, nMaxSize);
    ...
    if (pDir)
    {
        while (NULL != (ent=readdir(pDir)))
        {
            ...
            */
            ...
            //if(0 == str.compare(iPos + 1, strlen(szExName), szExName))
            {
                struct stat st;
                if (0 == stat(str.c_str(), &st))
                {
...
// Note：
//************************************
void CAdapter::EnumFile(const char* szFolderName, const char *szExName, std::vector<std::string>& vFileName, int nFileCount)
{
    //WINDOWS编译环境
#if (defined  MW_WINDOWS) || (defined  WIN32)
    CFileFind finder;
            ...
            */
            ...
            if(0 == str.compare(iPos, strlen(szExName), szExName))
            {
                ...
                if (0 == stat(str.c_str(), &st))
                {
					...
					//modified zwf 20160919 必须用mtime，不能用ctime
					if (tNow - st.st_mtime > 3600*24*nDay)
					{
						vFileName.push_back(str);						
					}
                }
...
BOOL CAdapter::IsFileModifyBeforeDay(const char* szFilePath, int nDay)
{
    struct stat st;
    if (0 == stat(szFilePath, &st))
    {
        time_t tNow = time(NULL);     
...
BOOL CAdapter::WritePrivateProfileString( LPCSTR lpAppName, LPCSTR lpKeyName, LPCSTR lpString, LPCSTR lpFileName )
{
	...
	while (!feof(pfile))
	{
		...
		//added by yby    末尾行必须跳出
		if(NULL == ReadString(sztmp, MAXLINELENGTH, pfile))
		{
			break;
		}		
		RemoveSpace(sztmp);
...
//设置线程的名称(暂只在LINUX版本有效，建议线程名称在15字节内)
void CAdapter::SetThreadName(const char* szThreadName)
{
#ifdef LINUX
...
bool CAdapter::IsDir(std::string& strPath)
{
    #if (defined  MW_WINDOWS) || (defined  WIN32)
      return true;
    #elif defined LINUX
    struct stat sb;  
    if (stat(strPath.c_str(), &sb) == -1) 
    {
        return false;  
    }
...
bool CAdapter::IsRegularFile(std::string& strPath)
{
    #if (defined  MW_WINDOWS) || (defined  WIN32)
        return true;
    #elif defined LINUX
