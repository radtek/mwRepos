...
//十六进制字符转换成整数
int CEncrypt::HexchartoInt(char chHex)
{
	int iDec = 0;
	switch (chHex)
	{
...
int CEncrypt::HexStrToMemBytes(std::string& strDest, const std::string& strSrc)
{
    int nReadLen = strSrc.length();
	strDest.resize(nReadLen/2+1);
    for (int i = 0, k = 0; i < nReadLen; i += 2)
    {
        strDest[k++] = HexchartoInt(strSrc[i]) * 16 + HexchartoInt(strSrc[i+1]);
    }
    return nReadLen/2;
}

//仅适用于可见字符串加密, 密码必须为16字节,返回小写的16进制字符串
//二进制加密请用Encrypt
//返回>0:成功,表示密文的长度(16的整数倍)，返回-1:错误。
int CEncrypt::EncryptReturnHexString(unsigned char *ucInput, unsigned int nInputLen, unsigned char *ucKey, std::string& strOutHexStr/*密文*/)
{
	int nSize = (nInputLen / 16 + 1) * 16;
	std::string strReserve = "";
	strReserve.resize(nSize + 1);
	int nLen = Encrypt(ucInput, nInputLen, ucKey, (unsigned char*)strReserve.data());
	int nHexLen = nLen;
	if (nLen > 0)
	{
		nHexLen = 0;
		strOutHexStr.reserve(nLen*2+1);
		char* p = (char*)strOutHexStr.data();
		char szTmp[2] = {0};
		for (int i = 0; i < nLen; ++i)
		{
			sprintf(szTmp, "%02x", (unsigned char)strReserve[i]);
			strOutHexStr.append(szTmp);
		}
	}
	return nHexLen=strOutHexStr.length();
}

//用法与Decrypt相同,返回string类型的原文
int CEncrypt::DecryptReturnString(const std::string& strIn, unsigned char *ucKey, std::string& strOutStr/*原文*/)
{
	std::string strOut = "";
	int nLen = HexStrToMemBytes(strOut, strIn);
	strOutStr.resize(nLen + 1);
	int nOutLen = Decrypt((unsigned char*)strOut.c_str(), nLen, ucKey, (unsigned char*)strOutStr.data());
	if (nOutLen > 0)
	{
		strOutStr.resize(strlen(strOutStr.c_str()));
	}
	return strOutStr.length();
}
