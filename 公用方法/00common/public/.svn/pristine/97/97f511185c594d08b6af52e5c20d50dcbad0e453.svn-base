...
public:
	...
	//用法与Encrypt相同,返回小写的16进制字符串
	int EncryptReturnHexString(unsigned char *ucInput, unsigned int nInputLen, unsigned char *ucKey, std::string& strOutHexStr/*密文*/);

	//ucInput开辟的缓冲大小尽量是16的整数倍
	//nInputLen必须是16的整数倍
	//ucOutput最终返回长度可能不是原长度，因为后面有补0
	//ucKey的长度必须为16字节
	//返回>0:成功,表示明文的长度，返回-1:错误。
	...
	int DecryptReturnString(const std::string& strIn, unsigned char *ucKey, std::string& strOutStr/*原文*/);
...
private:
	...
	int HexStrToMemBytes(std::string& strDest, const std::string& strSrc);
