...
bool CMySqlConnPool::InitPool(std::string& strErrMsg, std::string& strErrDesc, const char* szHost, const char* szDatabase, const char* szUser, const char* szPassword, unsigned int nPort, const char* szActiveSql, int nTimeOut, bool bUseUtf8, int nMinSize, int nMaxSize)
{
	bool bRet = true;
...
bool CMySqlConnPool::ApplayNewConn(DBCONN& conn, std::string& strErrMsg, std::string& strErrDesc)
{
	...
	try
	{
		...
		if (NULL != conn.pConn)
		{
			if(conn.pConn->Open(m_strHost.c_str(), m_strDataBase.c_str(), m_strUser.c_str(), m_strPwd.c_str(), m_nPort, m_bUseUtf8))
			{
...
bool CMySqlConnPool::TestConn(DBConnection* pConn)
{
	...
	try
	{
		if (NULL != pConn)
		{
			...
			{
				bRet = pConn->Open(m_strHost.c_str(), m_strDataBase.c_str(), m_strUser.c_str(), m_strPwd.c_str(), m_nPort, m_bUseUtf8);
			}
		}
	}
	catch (...)
...
bool CMySqlConnPoolMgr::CreateConnPool(std::string& strErrMsg, std::string& strErrDesc, const char* szPoolName, const char* szHost, const char* szDatabase, const char* szUser, const char* szPassword, unsigned int nPort, const char* szActiveSql, int nTimeOut, bool bUseUtf8, int nMinSize, int nMaxSize)
{
	bool bRet = false;
	CMySqlConnPool *pPool = new CMySqlConnPool();
	if (NULL != pPool)
	{
		if (pPool->InitPool(strErrMsg, strErrDesc, szHost, szDatabase, szUser, szPassword, nPort, szActiveSql, nTimeOut, bUseUtf8, nMinSize, nMaxSize))
		{
			SafeLock safelock(&m_csLock);
...
void* CMySqlConnPoolMgr::ThreadCheckConnPool(void* pParam)
{
	...
	prctl(PR_SET_NAME, "CheckConnPool");
