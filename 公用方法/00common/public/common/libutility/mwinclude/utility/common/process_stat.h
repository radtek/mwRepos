...
int UTILITY_Export get_cpu_usage();


// 获取当前进程内存和虚拟内存使用量，返回-1失败，0成功
int UTILITY_Export get_memory_usage(uint64_t* mem, uint64_t* vmem);


// 获取当前进程总共读和写的IO字节数，返回-1失败，0成功
int UTILITY_Export get_io_bytes(uint64_t* read_bytes, uint64_t* write_bytes);

//取磁盘空余量
int UTILITY_Export get_disk_free_space(const char* driver);
