...
//返回值：>=0:密文的长度 <0:失败
int UTILITY_Export MW_AESEncodePwd(const char* szKey, const char* szSrcStr, char* szDestStr);

//入参：szKey      密钥(注意:只支持明文字符串,且必须是16个字节)
//入参：szSrcStr   密文字符串(16进制的字符串)      
//出参：szDestStr  解密后的字符串
//返回值：>=0:明文的长度 <0:失败
int UTILITY_Export MW_AESDecodePwd(const char* szKey, const char* szSrcStr, char* szDestStr);
