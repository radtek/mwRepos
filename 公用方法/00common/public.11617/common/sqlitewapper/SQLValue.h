#ifndef __SQLITE_SQLVALUE_H__
#define __SQLITE_SQLVALUE_H__

#include "SQLTypes.h"

namespace SQLITE_SQLVALUE_NSAMESPACE {
...
public:
    CSQLValue();
    CSQLValue(const char* pData, unsigned long nLength, SQLITE_SQLTYPES_NSAMESPACE::Type eType=SQL_TEXT);
    CSQLValue(const CSQLValue& rValue);
    virtual ~CSQLValue();
protected:
    char* m_pData;
    unsigned long m_nLength;
    SQLITE_SQLTYPES_NSAMESPACE::Type   m_eType;
public:
    ...
    inline SQLITE_SQLTYPES_NSAMESPACE::Type GetType() const
    {
        return m_eType;
    }
