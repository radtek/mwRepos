#ifndef __ODBC_SQLVALUE_H__
#define __ODBC_SQLVALUE_H__

#include "SQLTypes.h"

namespace ODBC_SQLVALUE_NSAMESPACE {
...
public:
    CSQLValue();
    CSQLValue(const char* pData, unsigned long nLength, ODBC_SQLTYPES_NSAMESPACE::Type eType=SQL_CHAR);
    CSQLValue(const CSQLValue& rValue);
    virtual ~CSQLValue();
protected:
    char* m_pData;
    unsigned long m_nLength;
    ODBC_SQLTYPES_NSAMESPACE::Type   m_eType;
public:
    ...
    inline ODBC_SQLTYPES_NSAMESPACE::Type GetType() const
    {
        return m_eType;
    }
