...
void ODBC_SQLRESULT_NSAMESPACE::CSQLResult::Init(ODBC_SQLTYPES_NSAMESPACE::ConnectionPtr pCon,ODBC_SQLTYPES_NSAMESPACE::EnvironmentPtr pEnv)
{
    m_hCon    = pCon;
}
...
void ODBC_SQLRESULT_NSAMESPACE::CSQLResult::ResizeColumArr(int nParamIndex)
{
	if (nParamIndex > m_nParamNum)
	{
		...
		for (int i = 0; i < nOffSet; ++i)
		{
			...
			ColumInfo.pIndArr = new SQLINTEGER[m_nMaxIterations];
			memset(ColumInfo.pIndArr, 0, sizeof(SQLINTEGER)*m_nMaxIterations);
...
//1.设置SQL语句(同时申请stmt)
void ODBC_SQLRESULT_NSAMESPACE::CSQLResult::SetSQL(const char* szSql)
{
	//检查数据库是否已打开
    ...
    {	
		...
		if (nIterations > 0 && nMaxIterations > nIterations)
		{
			...
			for (it; it != m_vColumArrInfo.end(); ++it)
			{
				...
				{
					delete it->pIndArr;
					it->pIndArr = new SQLINTEGER[m_nMaxIterations];
					memset(it->pIndArr, 0, sizeof(SQLINTEGER)*m_nMaxIterations);
				}
...
void ODBC_SQLRESULT_NSAMESPACE::CSQLResult::SetParamNum(unsigned int nParamNum)
{
    ...
    {		
		...
		for (int i = 0; i < m_nMaxParamNum; ++i)
		{
			...
			columarrinfo.pIndArr = new SQLINTEGER[m_nMaxIterations];
			memset(columarrinfo.pIndArr, 0, sizeof(SQLINTEGER)*m_nMaxIterations);
...
{	
    ...
    {	
		...
		SQLINTEGER  sql_indicator = 0;
		SQLUINTEGER cbColDef = 0;
		SQLINTEGER  cbValueMax = 0;
		switch (sql_type)
		{
		case SQL_BIGINT:
		...
		*/
		...
		{
			m_vColumArrInfo[nIndex].pIndArr[i] = sql_indicator;
		}
		//预分配空间
		//1.内存空间不存在,直接重新分配
...
int ODBC_SQLRESULT_NSAMESPACE::CSQLResult::Query(const char* szSql, ODBC_SQLTYPES_NSAMESPACE::SQLSTRTYPE nType, bool bNeedRetSet)
{
    int nRet = -1;
    if (NULL == szSql)
    {
...
bool ODBC_SQLRESULT_NSAMESPACE::CSQLResult::Fetch(ODBC_SQLVALUE_NSAMESPACE::SQLValueArray& arrValues, bool bFetchBLOB)
{
    arrValues.clear();
    bool bRet = false;
    ThrowExceptionIfNotOpen();
...
void ODBC_SQLRESULT_NSAMESPACE::CSQLResult::GetFields()
{
    m_SQLFIELD.clear();
    ThrowExceptionIfNotOpen();
    ThrowExceptionIfNoResultSet();
...
struct PartStruct 
{
	...
	SQLINTEGER    TmInd;
	SQLINTEGER    BigIntInd;
};
SQLINTEGER arrPartID[ARRAY_SIZE];
SQLINTEGER arrPartIDInd[ARRAY_SIZE];
...
int  ODBC_SQLRESULT_NSAMESPACE::CSQLResult::Test()
{
	...
	SQLINTEGER* pArrInd = new SQLINTEGER[ARRAY_SIZE];
	...
	// column-wise binding.
	//SQLINTEGER nSize1 = SQL_PARAM_BIND_BY_COLUMN;
	...
	//SQLINTEGER nSize2 = sizeof(PartStruct);
