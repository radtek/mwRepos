...
struct COLUM_ARR_INFO
{
	...
	SQLINTEGER* pIndArr;				//IND数组指针

	SQLSMALLINT sql_c_type;			//c类型	
	SQLSMALLINT sql_type;			//DB中字段类型
	SQLINTEGER  cbColDef;			//列定义的大小
	SQLINTEGER	cbValueMax;			//列值的最大值
...
public:
   ...
   void Init(ODBC_SQLTYPES_NSAMESPACE::ConnectionPtr pCon,ODBC_SQLTYPES_NSAMESPACE::EnvironmentPtr pEnv);
   CSQLResult();
   virtual ~CSQLResult();
protected:
...
public:
   ...
   int  Query(const char* szSql, ODBC_SQLTYPES_NSAMESPACE::SQLSTRTYPE nType=ODBC_SQLTYPES_NSAMESPACE::SQLSTRTYPE_NORMAL, bool bNeedRetSet=true);
   bool Fetch(ODBC_SQLVALUE_NSAMESPACE::SQLValueArray& arrValues, bool bFetchBLOB = true);
   void EndFetch();
//绑定变量
public:
   //1.设置SQL语句(同时申请stmt)
   void SetSQL(const char* szSql=NULL);
